...
const Settings: React.FC<SettingsProps> = ({ settings, updateSettings, accounts, setAccounts, categories, setCategories, onRemoveInventoryTag, onFetchCloud, cloudStatus }) => {
  ...
  return (
    <div className="space-y-10 animate-slide-up pb-40 max-w-2xl mx-auto px-2 relative">
      ...
      <section className="space-y-4">
         ...
         {isAddingAdjustment && (
           <div className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-indigo-500/10 space-y-4 animate-in zoom-in-95 duration-300">
              <input value={newAdj.label} onChange={e => setNewAdj({...newAdj, label: e.target.value})} placeholder="Rule Name" className="w-full bg-slate-50 dark:bg-slate-800 rounded-2xl px-5 py-4 font-bold border-none text-slate-900 dark:text-white" />
              <div className="grid grid-cols-2 gap-4">
                 <select value={newAdj.type} onChange={e => setNewAdj({...newAdj, type: e.target.value as any})} className="bg-slate-50 dark:bg-slate-800 rounded-2xl px-5 py-4 font-bold border-none text-[10px] uppercase text-slate-900 dark:text-white">
                    <option value="FIXED">Amount</option>
                    <option value="PERCENT">Percentage %</option>
                 </select>
                 <input type="number" value={newAdj.value} onChange={e => setNewAdj({...newAdj, value: parseFloat(e.target.value) || 0})} placeholder="Value" className="bg-slate-50 dark:bg-slate-800 rounded-2xl px-5 py-4 font-bold border-none text-slate-900 dark:text-white" />
              </div>
         ...
         <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] overflow-hidden premium-shadow">
            {settings.pricingRules.customAdjustments.map((adj) => (
              <div key={adj.id} className="flex items-center justify-between p-6 border-b border-slate-50 dark:border-white/5">
                 <span className="font-black text-sm text-slate-900 dark:text-white">{adj.label}</span>
      ...
      <section className="space-y-4">
         <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-2">Business Taxonomy Control</p>
         <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] p-8 premium-shadow space-y-6">
            ...
            <div className="flex gap-2">
               <input value={newCategory} onChange={e => setNewCategory(e.target.value)} placeholder={`New ${activeTaxonomyTab} Label...`} className="flex-1 bg-slate-50 dark:bg-slate-800 rounded-2xl px-5 py-4 text-xs font-black border-none text-slate-900 dark:text-white" />
            ...
            <div className="flex flex-wrap gap-2">
               {(activeTaxonomyTab === TransactionType.TRANSFER ? settings.inventoryCategories : categories[activeTaxonomyTab] || []).map(cat => (
                 <div key={cat} className="flex items-center gap-3 bg-slate-50 dark:bg-slate-800 px-4 py-2.5 rounded-2xl border border-black/5">
                    <span className="text-[10px] font-black uppercase tracking-tight text-slate-700 dark:text-slate-300">{cat}</span>
                    <div className="flex gap-2">
                       {/* EDIT BUTTON (PENCIL) - Now hamesha visible and larger */}
                       <button onClick={() => { setRenamingTag({ old: cat, type: activeTaxonomyTab === TransactionType.TRANSFER ? 'INV' : activeTaxonomyTab }); setRenameValue(cat); }} className="text-indigo-500 hover:scale-110 transition-transform p-1">
                       ...
                       {/* DELETE BUTTON */}
                       <button onClick={() => activeTaxonomyTab === TransactionType.TRANSFER ? onRemoveInventoryTag(cat) : setCategories({...categories, [activeTaxonomyTab]: categories[activeTaxonomyTab].filter(c => c !== cat)})} className="text-rose-400 hover:text-rose-600 p-1 font-black text-lg">×</button>
                    </div>
                 </div>
               ))}
            </div>
      ...
      {renamingTag && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-6 animate-in fade-in duration-300">
          ...
          <div className="relative w-full max-w-xs bg-white dark:bg-slate-900 rounded-[3rem] p-8 shadow-2xl border border-white/10 animate-in zoom-in-95 duration-300">
             <h3 className="text-lg font-black uppercase mb-4 tracking-tightest text-slate-900 dark:text-white">Rename Label</h3>
             <input value={renameValue} onChange={e => setRenameValue(e.target.value)} className="w-full bg-slate-50 dark:bg-slate-800 rounded-2xl p-4 font-bold text-sm border-none mb-6 text-slate-900 dark:text-white" autoFocus />
             <div className="flex gap-2">
